...
```{r scatter}
ggplot(data = wine, aes(x=phenols, y=flavanoids, colour = origin, shape = origin)) + geom_point()
```
...
```{r variable-check}
str(wine)
#Some are integers rather than numeric continuous
```
...
```{r train-index}
train.index <- createDataPartition(wine$origin, 0.75, list = FALSE, times = 1)
```
...
```{r test}
wine.test <- dplyr::slice(wine.train, -1)
```
...
```{r train-pre}
train.proc <- preProcess(wine.train)
...
```{r train-knn}
train.knn <- train(origin ~ .,method = "knn", train.transformed)
...
# Prediction accuracy is almost 100% accurate. It is 97%
# Cross validation
