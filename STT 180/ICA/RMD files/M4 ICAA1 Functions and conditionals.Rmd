...
viewr_1 <- function(df, nrow, ncol) {
  df %>% 
    select(colnames(diamonds)[1:ncol]) %>% 
    sample_n(size=nrow)
}
```
...
viewr <- function(df, nrow, ncol) {
  stopifnot(ncol <= dim(df)[2], nrow <= dim(df)[1])
  df %>% 
    select(colnames(diamonds)[1:ncol]) %>% 
    sample_n(size=nrow)
}
...
```{r summary-fcn}
summary <- function(vect) {
  summation <- sum(vect)
  average <- mean(vect)
  message <- ""
  if (summation/average >= 0){
    message <- "sum/mean is +"
  }else {
    message <- "sum/mean is -"
  }
  return(c(sum(vect), mean(vect), message))
}
```
